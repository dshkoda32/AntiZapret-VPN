...
if [[ -f /etc/openvpn/server/scripts/client-connect.sh ]]; then
  echo '#!/bin/bash
...
exit 0' > /etc/openvpn/server/scripts/client-connect.sh
  chmod +x /etc/openvpn/server/scripts/client-connect.sh
fi
